...
Factor::Connector.service 'newrelic_metrics' do
  action 'summarize' do |params|
    ...
    fail "Metrics (metrics) values must all be arrays of string" unless metrics.values.all?{|value| value.is_a?(String) && value.all?{|i| i.is_a?(String)} }
    ...
    if range
      ...
      fail "Range (range) 'from' time is invalid" unless Cronic.parse(range['from'])
      if range['to']
        fail "Range (range) 'to' time must be a String" unless range['to'].is_a?(String)
        fail "Range (range) 'to' time is invalid" unless Cronic.parse(range['to'])
      end
    end

    option = {}
    option[:server] = server_id if server_id
    option[:application] = application_id if applicaiton_id
